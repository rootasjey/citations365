...
namespace BackgroundTasks {
    ...
    public sealed partial class AppShell : Page
    {
        ...
        private List<NavMenuItem> navlist = new List<NavMenuItem>(
            new[]
            {
                new NavMenuItem()
                {
                    SymbolAsChar = '\uE706',
                    Label = "Aujourd'hui",
                    DestinationPage = typeof(Views.TodayPage)
                },
                new NavMenuItem()
                {
                    SymbolAsChar = '\uE00B',
                    Label = "Favoris",
                    DestinationPage = typeof(Views.FavoritesPage)
                },
                new NavMenuItem()
                {
                    SymbolAsChar = '\uE11A',
                    Label = "Recherche",
                    DestinationPage = typeof(Views.SearchPage)
                },
                new NavMenuItem()
                {
                    SymbolAsChar = '\uE2AF',
                    Label = "Auteurs",
                    DestinationPage = typeof(Views.AuthorsPage)
                },
                //new NavMenuItem()
                //{
                //    Symbol = Symbol.Link,
        ...
        public AppShell()
        {
            ...
            // Tell the frame to navigate to the TodayPage
            frame.Navigate(typeof(Views.TodayPage));
        }
        ...
        {
            this.RootSplitView.IsPaneOpen = false; // close the splitview
        ...
        private void SettingsButton_Click(object sender, RoutedEventArgs e)
        {
            if (this.AppFrame.CurrentSourcePageType != typeof(SettingsPage))
            {
                ...
                NavMenuList.SetSelectedItem(null); // un-select the NavMenuListView selection if any
        ...
        /// <summary>
        /// Set the settings button's background to the AccentColor
        /// </summary>
        private void FocusSettingsButton()
        {
            // Get the current AccentColor
            Color accentColor = (Color)Application.Current.Resources["SystemAccentColor"];
            SolidColorBrush backColor = new SolidColorBrush(accentColor);
            SettingsButton.Background = backColor;
        }

        /// <summary>
        /// Reset the settings button's background
        /// </summary>
        private void UnFocusSettingsButton()
        {
